...
```mermaid
flowchart TD
    A[Matriz / Administração Central] --> B[Filial 1]
    A -->|Define| E[Padrões e Processos]
    A -->|Controla| F[Financeiro Consolidado]
...
```mermaid
flowchart TD
    Owner[Proprietário / Diretor] --> Financeiro[Responsável Financeiro]
    Owner --> Operacoes[Responsável de Operações]
    Owner --> Marketing[Responsável de Marketing]
    Operacoes --> U1[Gerente Unidade 1]
    Operacoes --> U2[Gerente Unidade 2]
    Operacoes --> U3[Gerente Unidade 3]
    U1 --> P1[Professores]
    U1 --> R1[Recepção]
    U2 --> P2[Professores]
    U2 --> R2[Recepção]
...
```mermaid
flowchart LR
    A[Divulgar vaga] --> B[Triagem de currículo/graduação]
    B --> C[Aula-teste avaliada]
    C --> D[Entrevista com proprietário/gerente]
    D --> E[Definir modelo de remuneração]
    E --> F[Contrato assinado]
    F --> G[Período de experiência 30-90 dias]
    G --> H[Efetivação]
```
...
```mermaid
flowchart TD
    A[Vencimento do plano] --> B{Pagou em ate 3 dias?}
    B -->|Sim| C[Encerrado]
    B -->|Nao| D[Mensagem automatica de lembrete]
    D --> E{Pagou em ate 7 dias?}
    E -->|Sim| C
    E -->|Nao| F[Contato pessoal - recepcao ou professor]
    F --> G{Pagou em ate 15 dias?}
    G -->|Sim| C
    G -->|Nao| H[Bloqueio de acesso as aulas]
    H --> I{Pagou em ate 30 dias?}
    I -->|Sim| C
    I -->|Nao| J[Negociacao ou encerramento do vinculo]
```
...
```mermaid
flowchart TD
    A[Aluno reclama] --> B[Ouvir sem interromper]
    B --> C[Confirmar entendimento do problema]
    C --> D{Pode resolver na hora?}
    D -->|Sim| E[Resolver e confirmar com o aluno]
    D -->|Nao| F[Explicar prazo e proximo passo]
    F --> G[Escalar para gerente/proprietario se necessario]
    G --> H[Retornar ao aluno com solucao ou posicao]
```
...
```mermaid
flowchart TD
    A[Pessoa ve conteudo/anuncio/indicacao] --> B[Agenda aula experimental]
    B --> C[Compareceu a aula experimental]
    C --> D[Recebeu proposta de matricula]
    D --> E[Matriculado]
    E --> F[Aluno ativo e fidelizado]
```
...
```mermaid
flowchart LR
    A[Nunca ouviu falar] --> B[Ve um post/anuncio/indicacao]
    B --> C[Pesquisa no Google/Instagram]
    C --> D[Entra em contato - WhatsApp/DM]
    D --> E[Agenda aula experimental]
    E --> F[Compareceu a aula]
    F --> G[Recebe proposta de matricula]
    G --> H[Aluno matriculado]
    H --> I[Aluno fidelizado e indicador]
```
...
```mermaid
flowchart LR
    A[Pessoa ve anuncio frio] --> B[Clica mas nao converte]
    B --> C[Entra em publico de remarketing]
    C --> D[Recebe novo anuncio - oferta ou prova social]
    D --> E[Converte em lead]
```
...
```mermaid
flowchart TD
    A[Contato recebido] --> B[Entender objetivo da pessoa]
    B --> C[Indicar modalidade e turma adequada]
    C --> D[Convidar para aula experimental]
    D --> E[Confirmar presenca 1 dia antes]
    E --> F[Aula experimental realizada]
    F --> G[Apresentar planos]
    G --> H{Fechou na hora?}
    H -->|Sim| I[Matricula concluida]
    H -->|Nao| J[Follow-up em 24-48h]
```
